...
uint32_t* volatile nextPagingBase;
uint32_t*** kernelPD;

void initPaging(){
	nextPagingBase = (uint32_t* volatile)0x400000;
...
uint32_t* allocPagingStruct() {
	...
	for (uint32_t* i = nextPagingBase; i < nextPagingBase + 0x1000; i++) *i = 0;
	return nextPagingBase;
}
